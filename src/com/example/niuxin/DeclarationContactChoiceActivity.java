...
import com.example.niuxin.DeclarationModelChoiceActivity.GetTemplateThread;
...
public class DeclarationContactChoiceActivity extends Activity {
	...
	protected void onResume() {
		...
		SearchThread gt = new SearchThread();
		gt.start();
	}
	@Override
	...
	public void init() {
		...
		Integer selectContractId = intent.getIntExtra("selectContractId",-1);
		
		if(null!=list){
			for (int i = 0; i < list.size(); i++) {// 对合约类型进行循环，获取选中的初始化
				Map<String, Object> map = new HashMap<String, Object>();
				map = list.get(i);
				Integer contractid =Integer.valueOf( map.get("id").toString());
				if (contractid==selectContractId) {// 如果传来的数据与其中的一条数据符合则设置checkbox为选中状态，获取到相应的数据
