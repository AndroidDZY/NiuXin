...
import com.example.niuxin.DeclarationUsershowActivity.TestThread;
...
public class DeclarationQunzushowActivity extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//事件监听
		Intent in = getIntent();
		intentgroupist = in.getStringExtra("intentgroup");
		
				
				if(null!=intentgroupist&&(!"".equals(intentgroupist))){
  ...
  //获取listview视图对象 
	...
	class TestThread extends Thread {
		...
		public void run() {
			...
			if(null!=mData)
				mData.clear();
			String[] sts = intentgroupist.split(",");
			for (int i = 0; i < jsonArray.length(); i++) {
				try {
					...
					if(null!=sts){
						for(String s:sts){
							if (chattype == 1&&(Integer.valueOf(s)==id)) {
								mData.add(map);						
							}
						}
					}
